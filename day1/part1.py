...
Your actual left and right lists contain many location IDs. _What is the total distance between your lists?
"""
...
def get_total_distance(lst_a, lst_b):
    lst_a, lst_b = sorted(lst_a), sorted(lst_b)
    distance = [abs(a - b) for a, b in zip(lst_a, lst_b)]
    return sum(distance)

def main():
    lst_a, lst_b = parse_input("input/d1_p1_input.txt")
    total_distance = get_total_distance(lst_a, lst_b)
